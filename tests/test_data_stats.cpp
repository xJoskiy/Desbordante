...
TEST(TestDataStats, TestGetNumberOfNullColumns) {
    ...
    algos::DataStats &stats = *stats_ptr;
    size_t num_nulls = stats.GetNumberOfNullColumns();
    EXPECT_EQ(num_nulls, 1);
}

TEST(TestDataStats, TestGetNumberOfColumnsWithNull) {
    auto stats_ptr = MakeStatPrimitive("TestMetric.csv", ',', false);
    algos::DataStats &stats = *stats_ptr;
    size_t num_nulls = stats.GetNumberOfColumnsWithNull();
    EXPECT_EQ(num_nulls, 10);
}
...
TEST(TestDataStats, TestGeometricMean) {
    auto test = [](int &&index) {
        auto stats_ptr = MakeStatPrimitive(test_file_name, ',', false);
        algos::DataStats &stats = *stats_ptr;
        auto geometric_mean_stat = stats.GetGeometricMean(index);
        auto geometric_mean = mo::Type::GetValue<mo::Double>(geometric_mean_stat.GetData());
...
TEST(TestDataStats, TestSumOfSquares) {
    auto stats_ptr = MakeStatPrimitive(test_file_name);
    algos::DataStats &stats = *stats_ptr;
    auto sum_stat = stats.GetSumOfSquares(7);
    auto sum = mo::Type::GetValue<mo::Double>(sum_stat.GetData());
...
TEST(TestDataStats, TestNumberOfNegatives) {
    auto stats_ptr = MakeStatPrimitive(test_file_name, ',', false);
    algos::DataStats &stats = *stats_ptr;
    auto num_negatives_stat = stats.GetNumberOfNegatives(8);
    auto num_negatives = mo::Type::GetValue<mo::Int>(num_negatives_stat.GetData());
