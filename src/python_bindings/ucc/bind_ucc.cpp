...
py::tuple SerializeUcc(UCC const& ucc) {
    py::tuple schema_state = table_serialization::SerializeRelationalSchema(ucc.GetSchema().get());
...
UCC DeserializeUcc(py::tuple t) {
    ...
    Vertical vert = table_serialization::DeserializeVertical(vertical_state, schema_ptr.get());
...
py::tuple MakeUCCNameTuple(UCC const& ucc) {
    ...
    return python_bindings::VectorToTuple(columns, [](auto elem) { return elem->GetName(); });
}
}  // namespace
...
void BindUcc(py::module_& main_module) {
    ...
    py::class_<UCC>(ucc_module, "UCC")
            ...
            .def(py::pickle(
                    // __getstate__
                    [](UCC const& ucc) { return SerializeUcc(ucc); },
                    // __setstate__
                    [](py::tuple t) { return DeserializeUcc(t); }));
